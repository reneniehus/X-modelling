...
model {
  ...
  // fixed priors
  // g ~ normal(3, 5);
  // TODO: fix g below, should not be hardcoded, this is an important prior
  g ~ normal(4, 5);
  Rho_country ~ lkj_corr(n_treatment);
  Rho_model ~ lkj_corr(n_treatment);
  // sigma_country ~ exponential(5);
  // sigma_model ~ exponential(5);
  // sigma ~ exponential(5);
  sigma_country ~ exponential(0.2);
  sigma_model ~ exponential(0.2);
