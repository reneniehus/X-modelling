// A script that takes S_init, prop_sever, and intervention impact as an input and obtains other init values (I_init, R_init).
data {
  ...
  real S_init_input; // initial proportion of susceptibles
  real prop_severe; // proportion of infections that are severe (aka ILIs?)
  int n_interventions; // number of different interventions (including baseline!)
  array[n_interventions] real interventions_impact; // observed hospitalisations
  int vacc_timing; // timing of the vaccination
}
transformed data {
...
transformed parameters {
  ...
  for (j in 1:n_scenario){
    ...
    S_ini_scen = S_init_input;
    //
      S_scen[1] = S_ini_scen;
    // I_scen[1] = I_ini_start[j];
    I_scen[1] = I_ini_start;
    ...
    for (t in 2:n_day_start) {
      ...
      severe_mean[t] = delta_severe[t] * pop_start;
...
model {
  ...
  // for (j in 1:n_scenario){
    for (t in 1:n_week_start) {
      severe_obs_start[t] ~ poisson( severe_mean_weekly_start[1,t] ) ;
    }
    // }
  // priors
  ...
  // logit(I_init) ~ normal( -10,5 );
  // I_ini_logit_start ~ normal( -10,5 ); //
    logit(I_init) ~ normal( -7,5 );
  I_ini_logit_start ~ normal( -7,5 ); //
}
generated quantities {
  ...
  I_ini_logit_prior = normal_rng( -7,5 );
      ...
      for (t in 1:n_day_full) {
        ...
        //
          if (t==1) { // set initial conditions
            ...
            gen_severe_mean[j,t] = gen_delta_severe[t]*pop_start ;
