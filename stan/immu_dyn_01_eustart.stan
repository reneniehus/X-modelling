...
transformed data {
  ...
  beta_scen[2] = beta + 0.10*(1-beta); // 75% less social distance/protection
  beta_scen[3] = beta; // baseline
  susc_red[1] = 0;
  susc_red[2] = 0;
  susc_red[3] = 0.2;
}
...
transformed parameters {
  ...
  array[n_scenario] real<lower=0, upper=1> prop_severe_start=inv_logit(prop_severe_logit_start);
  ...
  array[n_day_full] real<lower=0> severe_mean;
  ...
  // starting wave: has its own I_ini_start and prop_severe_start
  // but same beta 
  ...
  for (j in 1:n_scenario){
    ...
    for (t in 2:n_day_start) {
      ...
      real delta_infective_exposures;
      // end: local variables
      delta_infective_exposures = beta_j*S[t-1] *I[t-1];
      delta_S = -delta_infective_exposures;
      delta_I = delta_infective_exposures - I[t-1]*rate_infectious; 
      delta_R = I[t-1]*rate_infectious; 
      //
      ...
      R[t] = R[t-1] + delta_R;
      //
      delta_severe[t] = delta_infective_exposures * prop_severe_start[j] ; 
      severe_mean[t] = delta_severe[t]*pop_full;
    }
    // fill first position in other vectors
    severe_mean[1] = severe_mean[2];
...
model {
  ...
  prop_severe_logit_start ~ normal( 0,3 );
  
  logit(SIR_ini[1]) ~ normal( 1.386294,1 );
  logit(SIR_ini[2]) ~ normal( -9.21,2 );
  I_ini_logit_start ~ normal( -9.21,2 ); //
...
generated quantities {
  ...
  for (j in 1:n_scenario){
    ...
    for (t in 1:n_day_full) {
      ...
      //
      gen_delta_severe[t] = delta_infective_exposures * prop_severe_start[j] ; 
