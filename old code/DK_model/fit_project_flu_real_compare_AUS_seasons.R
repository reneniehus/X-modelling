# Script to compare how well parameters from pre-COVID seasons fit the 2022 season
...
mod2 <- cmdstan_model(stan_file='./stan/immu_dyn_02_eustart_v3.stan') # This compiles the script
...
aus_ili %>% filter(date>"2022-03-01") -> df_new
aus_ili %>% filter(date>"2016-03-01", date<"2017-01-01") -> df_aus

# ---- |-Plot: Australian wave + start of EU wave  ----
df_aus %>%  ggplot(aes(x=date,y= (100*value) )) + geom_line()
df_aus$value_infl = df_aus$value*100
df_aus$t_vw = c(1:nrow(df_aus))
...
df_new <- df_new %>%
  mutate(value_infl=100*value,
         value_infl_cumsum = cumsum(value_infl) ) %>%
  filter(value_infl > 150)

wave_start = df_new %>%
  filter(value_infl_cumsum<4000)
df_new$t_vw = c(1:nrow(df_new))
wave_start$t_vw = c(1:nrow(wave_start))

# df_aus %>% ggplot(aes(x=date,y=value_infl)) +
#   geom_point(col=NA)+
#   geom_point(data=wave_start,col="darkred")+
#   labs(subtitle ="So-far-observed EU wave (red)") -> p_eu;p_eu

wave_start %>% ggplot(aes(x=date,y=value_infl)) +
  geom_point()+
  #geom_point(data=wave_start,col="darkred")+
  labs(subtitle ="So-far-observed EU wave (red)") -> p_eu;p_eu
...
# Australia
df_aus %>% ggplot(aes(x=date,y=value_infl)) + geom_point(alpha=0.2) + 
  geom_point(alpha=1)+
...
pop_australia = 25.7 *1e6
pop_target = pop_australia
df_for_fit = df_aus
#wave_start = df_for_fit
stan_list = list(
...
fit02 %>% gather_draws(gen_severe_obs_full[t_vw]) %>% 
  mean_qi() %>% left_join(df_aus,by="t_vw") %>% 
  ggplot(aes(x=t_vw)) + 
...
fit02 %>% gather_draws(gen_severe_obs[scen,t_vw]) %>% filter(scen==1) %>% 
  ...
  #geom_point(data=df_new,aes(y=value_infl )) +
...
fit02 %>% 
  gather_draws(gen_severe_obs_full[t_vw]) %>% 
  mean_qi() %>%
  pull(.value) %>% 
  sum()

# Data
wave_start$value_infl %>% sum()
df_aus$value_infl %>% sum()
