...
inputData <- sqlQuery(channelSource, sqlCode) %>% 
  as.data.frame() %>% 
  pivot_longer(cols = c("S2015-2016","S2016-2017","S2017-2018","S2018-2019","S2021-2022"), names_to = "Season", values_to = "value")
...
wave_start = df_aus %>% mutate(value_infl=value_infl,
  ...
  filter(value_infl_cumsum<2e6)
# wave_start = inputData %>% 
#   filter(Season=="S2016-2017") %>%
#   mutate(value_infl=value,
#          value_infl_cumsum = cumsum(value_infl) ) %>% 
#   filter(value_infl_cumsum<300, 
#          value_infl_cumsum>3)
